...
const char* _TOMLToken_name(_TOMLToken_t token) {
    switch (token) {
        case _TOML_TABLE_HEADER: return "_TOML_TABLE_HEADER";
        case _TOML_TABLE_ARRAY_HEADER: return "_TOML_TABLE_ARRAY_HEADER";
        case _TOML_TABLE_BODY: return "_TOML_TABLE_BODY";
        case _TOML_TABLE_ARRAY_BODY: return "_TOML_TABLE_ARRAY_BODY";
        case _TOML_COMMENT: return "_TOML_COMMENT";
        case _TOML_KEY: return "_TOML_KEY";
        case _TOML_VALUE: return "_TOML_VALUE";
        case _TOML_STR_BASIC: return "_TOML_STR_BASIC";
        case _TOML_STR_LITERAL: return "_TOML_STR_LITERAL";
        case _TOML_STR_ML_BASIC: return "_TOML_STR_ML_BASIC";
        case _TOML_STR_ML_LITERAL: return "_TOML_STR_ML_LITERAL";
        case _TOML_ARRAY: return "_TOML_ARRAY";
        case _TOML_INLINE_TABLE: return "_TOML_INLINE_TABLE";
        default: return "UNKNOWN_TOKEN";
    }
}

YATL_Result_t _skipWS(_YATL_Cursor_t *cursor) {
  if (!cursor) return YATL_ERR_INVALID_ARG;
  if (!cursor->line) return YATL_ERR_INVALID_ARG;
...
YATL_Result_t _skipany(_YATL_Cursor_t *cursor, const char *chars) {
  if (!cursor) return YATL_ERR_INVALID_ARG;
  if (!cursor->line) return YATL_ERR_INVALID_ARG;
  if (!chars) return YATL_ERR_INVALID_ARG;
...
YATL_Result_t _consume(_YATL_Cursor_t *cursor, _TOMLToken_t token) {
  if (!cursor) return YATL_ERR_INVALID_ARG;
  if (!cursor->line) return YATL_ERR_INVALID_ARG;
  YATL_LOG(YATL_LOG_INFO, "Consuming token: %s", _TOMLToken_name(token));
  _YATL_Cursor_t cr = *cursor;
  ...
  switch (token) {
    ...
    case _TOML_TABLE_ARRAY_HEADER:
      while (cr.pos < cr.line->len) {
        char c = cr.line->text[cr.pos];
        if (c == ']' && cr.pos + 1 < cr.line->len && cr.line->text[cr.pos + 1] == ']') {
          cr.pos += 2;
          *cursor = cr;
          return YATL_OK;
        }
    ...
    case _TOML_VALUE: {
      // Consumes entire value including delimiters, cursor ends AFTER value
      ...
      if (c == '"' && c1 == '"' && c2 == '"') {
        ...
        if (res != YATL_OK) {
          YATL_LOG(YATL_LOG_WARN, "_TOML_VALUE: failed to consume multiline basic string");
          return res;
        }
      ...
      if (c == '\'' && c1 == '\'' && c2 == '\'') {
        ...
        if (res != YATL_OK) {
          YATL_LOG(YATL_LOG_WARN, "_TOML_VALUE: failed to consume multiline literal string");
          return res;
        }
        cr.pos += 3;  // skip closing '''
        *cursor = cr;
      ...
      if (c == '\'') {
        ...
        if (res != YATL_OK) {
          YATL_LOG(YATL_LOG_WARN, "_TOML_VALUE: failed to consume literal string");
          return res;
        }
        cr.pos++;  // skip closing '
        *cursor = cr;
    ...
    case _TOML_STR_ML_BASIC: {
      ...
      for (;;) {
        while (cr.pos < cr.line->len) {
          ...
          if (ch == '"' &&
              cr.pos + 1 < cr.line->len && cr.line->text[cr.pos + 1] == '"' &&
              cr.pos + 2 < cr.line->len && cr.line->text[cr.pos + 2] == '"') {
            *cursor = cr;
            return YATL_OK;
          }
          cr.pos++;
    ...
    case _TOML_ARRAY: {
      ...
      if (cr.line->text[cr.pos] != '[') return YATL_ERR_SYNTAX;
      int depth = 1;
      cr.pos++;
      for (;;) {
        while (cr.pos < cr.line->len) {
          ...
          if (ch == '"' && ch1 == '"' && ch2 == '"') {
            cr.pos += 3;
            _consume(&cr, _TOML_STR_ML_BASIC);
            cr.pos += 3;
            continue;
          }
          if (ch == '\'' && ch1 == '\'' && ch2 == '\'') {
            cr.pos += 3;
            _consume(&cr, _TOML_STR_ML_LITERAL);
            cr.pos += 3;
            continue;
          }
          if (ch == '"') {
            cr.pos++;
            _consume(&cr, _TOML_STR_BASIC);
            cr.pos++;
            continue;
          }
          if (ch == '\'') {
            cr.pos++;
            _consume(&cr, _TOML_STR_LITERAL);
            cr.pos++;
            continue;
          }
          if (ch == '[') depth++;
